...
const urlService = {
  ...
  // Update URL preview content
  updatePreview: async (urlId) => {
    try {
      const response = await api.post(`/urls/${urlId}/update_preview/`);
      return response.data;
    } catch (error) {
      console.error('Error updating preview content:', error.response?.data || error.message);
      throw error;
    }
  },
